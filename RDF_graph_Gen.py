...
Blank_keywords = ["Person", "Silhouette", "Brunette", "Creature", "Mortal", "Human Being", "Individual", "Character", "Personage"]
Ground_keywords = ["Liam", "Olivia", "Noah", "Emma", "Oliver", "Ava", "Elijah", "Charlotte", "William", "Sophia", "James",
                   "Amelia", "Benjamin", "Isabella", "Lucas", "Mia", "Henry", "Evelyn", "Alexander", "Harper"]
Predicate_keywords = ["Knows", "Enjoys", "Likes", "Hates", "Ignores", "Acknowledges", "Loves"]
...
######################################################
###
def generate_random_RDF_graph(number_of_blanks, number_of_ground_nodes, number_of_BG_edges, number_of_grounded_edges,ratio=0.7):
    blanks = {}
    ground_nodes = {}
    triplets = []
    ...
    for i in range(0,number_of_ground_nodes):
        Q =insert_random_keyword()+"".join(random.choices(string.ascii_uppercase + string.digits, k=6))+str(i)
    ...
    while _ < number_of_grounded_edges:
        ...
        if (T not in triplets):
            _ += 1
            triplets.append(T)
    return RDF.RDF_graph(blank_nodes = blanks, real_nodes = ground_nodes, triplets_collection = triplets)
